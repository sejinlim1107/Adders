    ...
    def construct_rounds(self):
        ...
        for t in range(1, int(mt.log2(n))+1):
            for m in range(self.l(n, t)):
                if t == 1: # B에 저장되어있는 애들로만 연산 가능
                    #print(int(mt.pow(2, t)*m + mt.pow(2, t-1)),2 * m + 1,int(mt.pow(2, t)*(m+1)))
        ...
        for t in range(int(mt.log2(2*n/3)),0,-1):
            for m in range(1,self.l((n-pow(2,t-1)), t)+1):
                if t == 1:  # B에 저장되어있는 애들로만 연산 가능
                    ...
                    c_round.append(cirq.TOFFOLI(self.Z[int(mt.pow(2, t) * m)], ancilla[len(ancilla)-1-self.l(n,t)-1+2*m],self.Z[int(mt.pow(2, t) * m + mt.pow(2, t - 1))]))
